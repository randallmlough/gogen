...
func TestPackages(t *testing.T) {
	t.Run("name for existing package does not load again", func(t *testing.T) {
		p := initialState(t)
		require.Equal(t, "a", p.NameForPackage("github.com/randallmlough/gogen/gocode/testdata/a"))
		require.Equal(t, 1, p.numLoadCalls)
	})

	t.Run("name for unknown package makes name only load", func(t *testing.T) {
		p := initialState(t)
		require.Equal(t, "c", p.NameForPackage("github.com/randallmlough/gogen/gocode/testdata/c"))
		require.Equal(t, 1, p.numLoadCalls)
	...
	t.Run("evicting a package causes it to load again", func(t *testing.T) {
		p := initialState(t)
		p.Evict("github.com/randallmlough/gogen/gocode/testdata/b")
		require.Equal(t, "a", p.Load("github.com/randallmlough/gogen/gocode/testdata/a").Name)
		require.Equal(t, 1, p.numLoadCalls)
		require.Equal(t, "b", p.Load("github.com/randallmlough/gogen/gocode/testdata/b").Name)
		require.Equal(t, 2, p.numLoadCalls)
	})

	t.Run("evicting a package also evicts its dependencies", func(t *testing.T) {
		p := initialState(t)
		p.Evict("github.com/randallmlough/gogen/gocode/testdata/a")
		require.Equal(t, "a", p.Load("github.com/randallmlough/gogen/gocode/testdata/a").Name)
		require.Equal(t, 2, p.numLoadCalls)
		require.Equal(t, "b", p.Load("github.com/randallmlough/gogen/gocode/testdata/b").Name)
...
func TestNameForPackage(t *testing.T) {
	...
	assert.Equal(t, "api", p.NameForPackage("github.com/randallmlough/gogen/api"))

	// does not contain go code, should still give a valid name
	assert.Equal(t, "docs", p.NameForPackage("github.com/randallmlough/gogen/docs"))
...
func initialState(t *testing.T) *Packages {
	p := &Packages{}
	pkgs := p.LoadAll(
		"github.com/randallmlough/gogen/gocode/testdata/a",
		"github.com/randallmlough/gogen/gocode/testdata/b",
	)
